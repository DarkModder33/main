...
import { loadUserProfile, saveUserProfile } from '../lib/personalizationEngine';

export default function Profile() {
  const [profile, setProfile] = useState(loadUserProfile());
  const [editing, setEditing] = useState(false);
  const [avatarUrl, setAvatarUrl] = useState(profile.avatarUrl || 'https://api.dicebear.com/7.x/identicon/svg?seed=tradehax');
    ...
    <div style={{ maxWidth: 600, margin: '40px auto', background: '#0E1117', color: '#C8D8E8', borderRadius: 12, padding: 32 }}>
      ...
      <div style={{ marginBottom: 18 }}>
        ...
        <input
          type="text"
          name="favoriteAssets"
          value={profile.favoriteAssets.join(', ')}
          onChange={e => setProfile(prev => ({ ...prev, favoriteAssets: e.target.value.split(',').map(a => a.trim()) }))}
          style={{ width: '100%', marginTop: 6, padding: 8, borderRadius: 6, border: '1px solid #1C2333', background: '#12161E', color: '#C8D8E8' }}
        />
      </div>
      <button
