...
export const MobileMenu = () => {
  ...
  return (
    <div className="md:hidden">
      ...
      {/* Drawer */}
      <div className={`fixed top-0 right-0 w-[85%] h-full z-[70] transform transition-transform duration-700 cubic-bezier(0.16, 1, 0.3, 1) bg-[rgba(10,12,16,0.9)] supports-[backdrop-filter]:bg-[rgba(10,12,16,0.86)] backdrop-blur-md border-l border-cyan-400/20 ${isOpen ? 'translate-x-0 shadow-[-20px_0_50px_rgba(6,182,212,0.16)]' : 'translate-x-full'}`}>
        ...
        <div className="p-8 h-full flex flex-col relative z-10">
          <div className="flex justify-between items-center mb-16">
            ...
            <button onClick={toggleMenu} className="text-zinc-200 hover:text-white transition-colors font-mono text-[11px] tracking-widest border border-white/20 px-3 py-1.5 rounded-full bg-white/10">
              [ CLOSE ]
            </button>
          </div>
          ...
          <nav className="flex flex-col gap-6">
            {navLinks.map((link, i) => (
              <Link
                ...
                href={link.href}
                onClick={toggleMenu}
                className="group relative rounded-xl border border-white/10 bg-white/[0.03] hover:bg-cyan-500/10 hover:border-cyan-400/30 transition-all"
              >
                <div className="absolute -left-4 top-1/2 -translate-y-1/2 w-0 h-px bg-cyan-500 group-hover:w-8 transition-all duration-500" />
                <span className="text-[16px] sm:text-[18px] font-black text-zinc-100 tracking-wide group-hover:text-cyan-300 transition-all uppercase italic px-4 py-3.5 flex items-center gap-3 transform group-hover:translate-x-1">
